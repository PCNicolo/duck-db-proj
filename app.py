...
from src.duckdb_analytics.ui.streaming_components import (
    ...
    VirtualScrollDataFrame,
    ProgressiveLoader,
    show_query_progress_indicator,
    create_cancel_query_button
)
...
def sql_editor_tab():
    ...
    use_enhanced = st.checkbox("Use Enhanced Editor (Beta)", value=True, key="use_enhanced_editor")
    ...
    if table_name:
        ...
        if "template_engine" not in st.session_state:
            from src.duckdb_analytics.analytics import (
                TemplateEngine, DataProfiler, FilterBuilder, 
                AnalyticsOptimizer, ProgressiveLoader
            )
        ...
        if selected_template_name:
            ...
            if st.button("🚀 Execute Template", type="primary"):
                try:
                    ...
                    if result['success']:
                        st.success(f"✅ Template executed successfully ({result['row_count']:,} rows)")
                        
                        # Store results in session state for other tabs
                        st.session_state.query_result = result['data']
                        st.session_state.execution_time = 0  # Template execution doesn't track time separately
...
def render_template_parameter(param, columns: List[Dict[str, Any]], table_name: str):
    ...
    from src.duckdb_analytics.analytics.templates import ParameterType, ColumnFilter
    ...
    if param.type == ParameterType.COLUMN:
        ...
        if not available_columns:
            st.warning(f"⚠️ No columns available for parameter '{param.label}' (requires {param.column_filter.value if param.column_filter else 'any'} type)")
            return None
...
def render_data_profiling(table_name: str):
    ...
    try:
        ...
        with col3:
            if st.button("📋 Full Profile"):
                with st.spinner("Generating comprehensive data profile..."):
                    try:
                        profile = st.session_state.data_profiler.profile_table(table_name, sample_size)
        ...
        if f"profile_stats_{table_name}" in st.session_state:
            ...
            for col_name, col_stats in stats['column_statistics'].items():
                ...
                if col_stats.get('type') == 'numeric':
                    summary_row['Mean'] = f"{col_stats.get('mean', 0):.2f}" if col_stats.get('mean') else ""
                    summary_row['Std Dev'] = f"{col_stats.get('std_dev', 0):.2f}" if col_stats.get('std_dev') else ""
                elif col_stats.get('type') == 'text':
                    summary_row['Avg Length'] = f"{col_stats.get('avg_length', 0):.1f}" if col_stats.get('avg_length') else ""
                elif col_stats.get('type') == 'boolean':
                    summary_row['True Ratio'] = f"{col_stats.get('true_ratio', 0)*100:.1f}%" if col_stats.get('true_ratio') else ""
                ...
                if selected_column:
                    ...
                    with st.expander(f"📋 Detailed Analysis: {selected_column}"):
                        ...
                        with col2:
                            ...
                            st.write(f"- Completeness: {col_stats.get('completeness', 0)*100:.1f}%")
                            st.write(f"- Uniqueness: {col_stats.get('uniqueness', 0)*100:.1f}%")
                            
                            if col_stats.get('outlier_count'):
                                st.write(f"- Outliers: {col_stats['outlier_count']} ({col_stats['outlier_count']/col_stats.get('count', 1)*100:.1f}%)")
        ...
        if f"full_profile_{table_name}" in st.session_state:
            ...
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                st.metric("Overall Quality", f"{overall_quality['total_score']*100:.1f}%")
            with col2:
            ...
            if profile.relationships:
                ...
                for relationship in profile.relationships:
                    if relationship['type'] == 'correlation':
                        st.write(f"📊 **{relationship['column1']}** and **{relationship['column2']}** have a "
...
def visualizations_tab():
    ...
    try:
        ...
        logger.error(f"Error accessing query_result in visualizations: {str(e)}", exc_info=True)
    ...
    if "chart_recommendation_engine" not in st.session_state:
        from src.duckdb_analytics.visualizations import (
            ChartRecommendationEngine, ChartExportManager, 
...
def render_chart_creation(df: pd.DataFrame):
    ...
    from src.duckdb_analytics.visualizations import (
        create_heatmap, create_treemap, create_sankey, create_box_plot,
    ...
    with col2:
        if st.button("🤖 Get Recommendations", help="Get AI-powered chart recommendations"):
    ...
    if chart_type == ChartType.HEATMAP:
        ...
        with col3:
            parents_col = st.selectbox("Parent Categories (optional)", 
                                     [None] + categorical_cols, key="treemap_parents")
        ...
        with col2:
            x_col = st.selectbox("Categories (optional)", 
                               [None] + categorical_cols, key="box_x")
        ...
        with col3:
            group_by = st.selectbox("Group By (optional)", 
                                  [None] + categorical_cols, key="radar_group")
...
def render_smart_recommendations(df: pd.DataFrame):
    ...
    if st.button("🔍 Analyze Data & Generate Recommendations", type="primary"):
        with st.spinner("Analyzing data patterns and generating recommendations..."):
            try:
                recommendations = st.session_state.chart_recommendation_engine.recommend_charts(df)
    ...
    if "chart_recommendations" in st.session_state:
        ...
        if recommendations:
            ...
            for i, rec in enumerate(recommendations):
                with st.expander(f"📊 Recommendation {i+1}: {rec.chart_type.value.title().replace('_', ' ')} (Score: {rec.score:.2f})"):
                    col1, col2 = st.columns([2, 1])
                    ...
                    with col2:
                        if st.button(f"Create This Chart", key=f"create_rec_{i}"):
                            # Create the recommended chart
                            st.session_state.selected_recommendation = rec
                            st.success("Chart configuration applied! Switch to 'Create Charts' tab to customize.")
        else:
...
def render_export_options():
    ...
    with col1:
        ...
        export_format = st.selectbox(
            "Format",
            ["PNG", "SVG", "HTML", "JSON", "PDF"],
            key="export_format"
        )
    
    with col2:
        st.write("**Export Settings**")
        options = st.session_state.chart_export_manager.get_export_options(export_format.lower())
    ...
    if st.button("📥 Export Chart", type="primary"):
        try:
            ...
            mime_type = st.session_state.chart_export_manager.get_mime_type(export_format.lower())
...
def data_explorer_tab():
    ...
    with col2:
        if st.button("🔄 Refresh Data"):
            ...
            cache_keys_to_clear = [key for key in st.session_state.keys() if table_name in str(key)]
    ...
    try:
        ...
        from src.duckdb_analytics.ui.relationship_visualizer import RelationshipVisualizer
        ...
        if 'column_statistics' not in st.session_state:
            st.session_state.column_statistics = ColumnStatisticsManager(st.session_state.db_connection)
    ...
    filter_tab, search_tab, stats_tab, pagination_tab, export_tab, relationships_tab = st.tabs([
        "🔧 Advanced Filters",
        "🔍 Column Search", 
        "📊 Statistics",
        "📄 Pagination",
    ...
    with filter_tab:
        ...
        filter_config = st.session_state.advanced_filter_builder.render_filter_builder(df)
        
        if filter_config['filters']:
            try:
                working_df = st.session_state.advanced_filter_builder.apply_filters(working_df, filter_config)
                st.success(f"✅ Filters applied: {len(working_df):,} rows remaining from {len(df):,} original rows")
            except Exception as e:
                st.error(f"❌ Error applying filters: {str(e)}")

    with search_tab:
        st.subheader("🔍 Column-Level Search")
        search_config = st.session_state.column_search_manager.render_column_searches(working_df)
        
        if search_config['searches']:
            try:
                working_df = st.session_state.column_search_manager.apply_column_searches(working_df, search_config)
                st.success(f"✅ Search applied: {len(working_df):,} rows match search criteria")
            except Exception as e:
    ...
    with stats_tab:
        ...
        try:
            stats_result = st.session_state.column_statistics.render_column_statistics_interface(
    ...
    with pagination_tab:
        ...
        try:
            pagination_result = st.session_state.smart_pagination.render_pagination_interface(
    ...
    with relationships_tab:
        ...
        try:
            relationship_result = st.session_state.relationship_visualizer.render_relationship_interface()
        except Exception as e:
        ...
        if use_streaming and estimated_time > 2.0:
            ...
            with progress_container:
                ...
                if not streamer.state.is_cancelled:
                    st.success(f"✅ Query completed in {execution_time:.3f}s ({len(df):,} rows)")
        else:
