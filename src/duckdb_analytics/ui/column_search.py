...
class ColumnSearchManager:
    ...
    def render_column_searches(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if len(df) > 10000:
            st.info(f"⚡ Searching in {len(df):,} rows. Large datasets may take longer.")
        ...
        columns_to_search = st.multiselect(
            ...
            df.columns.tolist(),
            default=list(st.session_state.column_searches.keys())[:5]  # Limit default selection
        )
        
        if columns_to_search:
            # Create tabs for each column type
            numeric_cols = [col for col in columns_to_search if pd.api.types.is_numeric_dtype(df[col])]
            text_cols = [col for col in columns_to_search if pd.api.types.is_string_dtype(df[col])]
            date_cols = [col for col in columns_to_search if pd.api.types.is_datetime64_any_dtype(df[col])]
            bool_cols = [col for col in columns_to_search if pd.api.types.is_bool_dtype(df[col])]
            ...
            if tabs:
                ...
                for i, tab_type in enumerate(tabs):
                    with tab_containers[i]:
                        if tab_type == "text":
                            search_configs.update(self._render_text_searches(df, text_cols, case_sensitive, show_suggestions))
                        elif tab_type == "numeric":
                            search_configs.update(self._render_numeric_searches(df, numeric_cols))
                        elif tab_type == "date":
                            search_configs.update(self._render_date_searches(df, date_cols))
                        elif tab_type == "bool":
                            search_configs.update(self._render_bool_searches(df, bool_cols))
        ...
        if search_configs:
            active_searches = sum(1 for config in search_configs.values() if config.get('active', False))
    ...
    def _render_text_searches(self, df: pd.DataFrame, columns: List[str], case_sensitive: bool, show_suggestions: bool) -> Dict[str, Any]:
        ...
        for col in columns:
            with st.expander(f"Search in '{col}'", expanded=len(columns) <= 3):
                ...
                with col1:
                    ...
                    current_search = st.session_state.column_searches.get(col, {}).get('value', '')
                ...
                if show_suggestions and search_value:
                    suggestions = self._get_text_suggestions(df[col], search_value, case_sensitive)
                    if suggestions:
                        st.write("💡 Suggestions:")
                        suggestion_cols = st.columns(min(3, len(suggestions)))
                        for i, suggestion in enumerate(suggestions[:3]):
                            with suggestion_cols[i]:
                                if st.button(f"'{suggestion}'", key=f"suggest_{col}_{i}"):
                                    st.session_state[search_key] = suggestion
                                    st.rerun()
                
                # Value frequency analysis
                if search_value:
                    matches = self._apply_text_search(df[col], search_value, search_type, case_sensitive, fuzzy_threshold)
                    match_count = matches.sum()
                    st.caption(f"Found {match_count:,} matches ({match_count/len(df)*100:.1f}% of rows)")
    ...
    def _render_numeric_searches(self, df: pd.DataFrame, columns: List[str]) -> Dict[str, Any]:
        ...
        for col in columns:
            with st.expander(f"Search in '{col}'", expanded=len(columns) <= 3):
                ...
                search_type = st.selectbox(
                    "Search Type",
                    ["Range", "Exact Value", "Greater Than", "Less Than", "Top N Values", "Bottom N Values"],
                    key=f"num_type_{col}"
                )
                ...
                if search_value is not None:
                    matches = self._apply_numeric_search(col_data, search_value, search_type)
    ...
    def _render_date_searches(self, df: pd.DataFrame, columns: List[str]) -> Dict[str, Any]:
        ...
        for col in columns:
            with st.expander(f"Search in '{col}'", expanded=len(columns) <= 3):
                ...
                if search_type == "Date Range":
                    ...
                    with col1:
                        start_date = st.date_input(
                            "Start Date",
                            value=pd.to_datetime(col_data.min()).date() if not col_data.empty else date.today(),
                            key=f"start_{col}"
                        )
                    with col2:
                        end_date = st.date_input(
                            "End Date",
                            value=pd.to_datetime(col_data.max()).date() if not col_data.empty else date.today(),
                            key=f"end_{col}"
                        )
                    ...
                    search_value = st.date_input(
                        "Select Date",
                        value=pd.to_datetime(col_data.median()).date() if not col_data.empty else date.today(),
                        key=f"specific_{col}"
                    )
                    ...
                    relative_type = st.selectbox(
                        "Relative to",
                        ["Today", "This Week", "This Month", "This Year", "Last N Days"],
                    ...
                    part_value = st.number_input(
                        f"{date_part} Value",
                        min_value=1,
                        max_value=12 if date_part == "Month" else (7 if date_part == "Weekday" else 31),
                        value=1,
                ...
                # Match count
                if search_value is not None:
                    matches = self._apply_date_search(col_data, search_value, search_type)
                    if hasattr(matches, 'sum'):
                        match_count = matches.sum()
                        st.caption(f"Found {match_count:,} matches ({match_count/len(df)*100:.1f}% of rows)")
    ...
    def _render_bool_searches(self, df: pd.DataFrame, columns: List[str]) -> Dict[str, Any]:
        ...
        for col in columns:
            with st.expander(f"Filter '{col}'", expanded=len(columns) <= 3):
                ...
                st.write(f"**Distribution:** ✅ {true_count:,} True | ❌ {false_count:,} False | ❓ {null_count:,} Null")
                
                # Filter options
                show_true = st.checkbox(f"Show True values", value=True, key=f"true_{col}")
                show_false = st.checkbox(f"Show False values", value=True, key=f"false_{col}")
                show_null = st.checkbox(f"Show Null values", value=True, key=f"null_{col}")
    ...
    def _render_search_history(self):
        ...
        if st.session_state.column_search_history:
            with st.expander("📈 Search History"):
                history_items = list(st.session_state.column_search_history.items())[-10:]  # Last 10 items
                ...
                for timestamp, search_data in reversed(history_items):
                    ...
                    with col3:
                        if st.button("Reuse", key=f"reuse_{timestamp}"):
                            # Restore search
                            st.session_state.column_searches[search_data['column']] = search_data
                            st.rerun()
    
    def _get_text_suggestions(self, series: pd.Series, search_term: str, case_sensitive: bool) -> List[str]:
        ...
        if not case_sensitive:
            ...
            matches = [val for val in unique_values 
                      if search_term in str(val).lower() and str(val).lower() != search_term.lower()]
        else:
            matches = [val for val in unique_values 
    ...
    def apply_column_searches(self, df: pd.DataFrame, search_config: Dict[str, Any]) -> pd.DataFrame:
        """
        ...
        for col, config in search_config['searches'].items():
            ...
            if config['type'] == 'text':
                ...
                mask = self._apply_numeric_search(df[col], config['value'], config['search_type'])
                result_df = result_df[mask]
            
            elif config['type'] == 'date':
                mask = self._apply_date_search(df[col], config['value'], config['search_type'])
                result_df = result_df[mask]
    ...
    def _apply_text_search(self, series: pd.Series, search_value: str, search_type: str, case_sensitive: bool, fuzzy_threshold: float) -> pd.Series:
    ...
    def _apply_numeric_search(self, series: pd.Series, search_value: Union[float, List[float], int], search_type: str) -> pd.Series:
    ...
    def _apply_date_search(self, series: pd.Series, search_value: Any, search_type: str) -> pd.Series:
    ...
    def _apply_boolean_search(self, series: pd.Series, search_value: Dict[str, bool]) -> pd.Series:
