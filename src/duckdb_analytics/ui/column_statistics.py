...
class ColumnStatisticsManager:
    ...
    def render_column_statistics_interface(self, df: pd.DataFrame, table_name: str = "data") -> Dict[str, Any]:
        """
        ...
        with st.expander("⚙️ Statistics Display Options"):
            ...
            with col1:
                ...
                show_distributions = st.checkbox(
                    "Show Distributions",
                    value=st.session_state.column_stats_preferences['show_distributions'],
            ...
            with col2:
                show_data_quality = st.checkbox(
                    "Show Data Quality",
                    value=st.session_state.column_stats_preferences['show_data_quality'],
    ...
    def _get_or_generate_column_stats(self, df: pd.DataFrame, table_name: str, config: Dict[str, Any]) -> Dict[str, Any]:
        """Get cached statistics or generate new ones."""
        cache_key = f"{table_name}_{len(df)}_{df.columns.tolist()}"
        
        # Check cache
        if (config['cache_enabled'] and 
            cache_key in st.session_state.column_stats_cache and 
            not config['auto_refresh']):
    ...
    def _generate_comprehensive_stats(self, df: pd.DataFrame, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for column in df.columns:
            stats['columns'][column] = self._analyze_single_column(df[column], column, config)
            ...
            if df[col].dtype == 'object':
                ...
                try:
                    ...
                    if numeric_count > len(df) * 0.8:
                        quality_issues.append(f"Column '{col}' appears numeric but stored as text")
                        quality_score -= 0.1
                except:
                    pass
    ...
    def _generate_quality_recommendations(self, issues: List[str]) -> List[str]:
        ...
        for issue in issues:
            if "missing data" in issue.lower():
                recommendations.append("Consider imputation strategies or explicit handling of missing values")
                ...
                recommendations.append("Convert numeric text columns to proper numeric types")
        
        return recommendations
    
    def _analyze_single_column(self, series: pd.Series, column_name: str, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _analyze_numeric_column(self, series: pd.Series, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(clean_series) > 0:
            ...
            stats['numeric_stats'] = {
                ...
                'kurtosis': float(clean_series.kurtosis()) if len(clean_series) > 3 else 0
            }
            ...
            stats['outliers'] = {
                'count': len(outliers),
                'percentage': len(outliers) / len(clean_series) * 100 if len(clean_series) > 0 else 0,
    ...
    def _analyze_text_column(self, series: pd.Series, config: Dict[str, Any]) -> Dict[str, Any]:
        """Analyze text column."""
        clean_series = series.dropna()
    ...
    def _analyze_datetime_column(self, series: pd.Series, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(clean_series) > 0:
            ...
            stats['temporal_patterns'] = {
                ...
                'hours': clean_series.dt.hour.value_counts().to_dict() if clean_series.dt.hour.nunique() > 1 else {}
            }
    ...
    def _analyze_boolean_column(self, series: pd.Series, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _analyze_column_quality(self, series: pd.Series) -> Dict[str, Any]:
        ...
        if len(series) > 1:
            ...
            if unique_pct < 0.01 and series.nunique() > 1:  # Very low uniqueness but not constant
        ...
        if pd.api.types.is_string_dtype(series):
            ...
            try:
                ...
                if 0.1 < numeric_convertible / len(series) < 0.9:  # Partial convertibility suggests inconsistency
    ...
    def _render_statistics_display(self, df: pd.DataFrame, column_stats: Dict[str, Any], config: Dict[str, Any]):
        """Render the statistics display interface."""
        # Overview section
        self._render_overview_section(column_stats['overview'], column_stats['data_quality'])
    ...
    def _render_overview_section(self, overview: Dict[str, Any], data_quality: Dict[str, Any]):
    ...
    def _render_grid_view(self, df: pd.DataFrame, column_stats: Dict[str, Any], selected_columns: List[str], config: Dict[str, Any]):
        ...
        for i in range(0, len(selected_columns), cols_per_row):
            ...
            for j, column in enumerate(selected_columns[i:i+cols_per_row]):
                with cols[j]:
                    self._render_column_card(column, df[column], column_stats['columns'][column], config)
    
    def _render_column_card(self, column_name: str, series: pd.Series, stats: Dict[str, Any], config: Dict[str, Any]):
        ...
        with st.container():
            ...
            type_emoji = {
                ...
                'datetime64[ns]': '📅', 'bool': '☑️', 'category': '🏷️'
            }
            ...
            if 'numeric_stats' in stats:
                st.write("**Range**")
                st.write(f"{stats['numeric_stats']['min']:.2f} → {stats['numeric_stats']['max']:.2f}")
                st.write(f"**Mean:** {stats['numeric_stats']['mean']:.2f}")
            
            elif 'text_stats' in stats:
                st.write("**Length Range**")
                st.write(f"{stats['text_stats']['min_length']} → {stats['text_stats']['max_length']} chars")
    ...
    def _render_detailed_view(self, df: pd.DataFrame, column_stats: Dict[str, Any], selected_columns: List[str], config: Dict[str, Any]):
        """Render detailed view for selected columns."""
        for column in selected_columns:
            with st.expander(f"📊 {column} - Detailed Analysis", expanded=len(selected_columns) == 1):
                self._render_detailed_column_analysis(column, df[column], column_stats['columns'][column], config)
    
    def _render_detailed_column_analysis(self, column_name: str, series: pd.Series, stats: Dict[str, Any], config: Dict[str, Any]):
    ...
    def _render_numeric_details(self, stats: Dict[str, Any], config: Dict[str, Any]):
        ...
        if 'outliers' in stats:
            ...
            if outliers['count'] > 0:
                st.warning(f"⚠️ {outliers['count']} outliers detected ({outliers['percentage']:.1f}%)")
                st.write(f"Outlier bounds: [{outliers['lower_bound']:.2f}, {outliers['upper_bound']:.2f}]")
    ...
    def _render_text_details(self, stats: Dict[str, Any], config: Dict[str, Any]):
        ...
        with col2:
            ...
            if 'patterns' in stats and stats['patterns']:
                ...
                for pattern_name, pattern_info in stats['patterns'].items():
                    ...
                    st.write(f"{pattern_display}: {pattern_info['count']} ({pattern_info['percentage']:.1f}%)")
        ...
        if 'top_values' in stats:
            ...
            for i, (value, count, pct) in enumerate(zip(top_values['values'][:5], 
                                                       top_values['counts'][:5], 
    ...
    def _render_datetime_details(self, stats: Dict[str, Any], config: Dict[str, Any]):
        ...
        with col2:
            ...
            if 'temporal_patterns' in stats:
                ...
                if patterns['years']:
                    st.write(f"Years: {min(patterns['years'])} - {max(patterns['years'])}")
                
                # Most common month
                if patterns['months']:
                    most_common_month = max(patterns['months'].items(), key=lambda x: x[1])
                    month_names = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun',
                                  'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
                    st.write(f"Most common month: {month_names[most_common_month[0]-1]}")
                
                # Most common day of week
                if patterns['days_of_week']:
                    most_common_day = max(patterns['days_of_week'].items(), key=lambda x: x[1])
    ...
    def _render_summary_table(self, df: pd.DataFrame, column_stats: Dict[str, Any], selected_columns: List[str]):
    ...
    def _render_distribution_visualization(self, series: pd.Series, stats: Dict[str, Any]):
