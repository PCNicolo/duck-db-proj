...
class RelationshipVisualizer:
    ...
    def _render_relationship_detection(self):
        ...
        if st.session_state.detected_relationships:
            ...
            for i, rel in enumerate(st.session_state.detected_relationships):
                with st.expander(
                    ...
                    with col2:
                        ...
                        st.write(f"**Sample Match Rate:** {rel.get('match_rate', 'N/A')}")
            ...
            st.info("No relationships detected. Try adjusting the detection method or confidence threshold.")
        ...
        if st.button("🎨 Generate Diagram", type="primary"):
            with st.spinner("Generating schema diagram..."):
                ...
                if diagram_fig:
                    ...
                    st.warning("Unable to generate diagram. Please ensure tables are loaded.")
    ...
    def _render_relationship_validation(self):
        ...
        if not all_relationships:
            st.info("No relationships to validate. Please detect or add relationships first.")
            return
        ...
        with col1:
            validation_method = st.selectbox(
                "Validation Method",
                ["Data Integrity Check", "Referential Integrity", "Statistical Analysis"],
        ...
        if st.session_state.relationship_validation_results:
            ...
            for rel_key, result in st.session_state.relationship_validation_results.items():
                ...
                with st.expander(f"{status_emoji} {rel_key}"):
                    ...
                    with col1:
                        st.write(f"**Status:** {'Valid' if result['valid'] else 'Invalid'}")
                        st.write(f"**Integrity Score:** {result['integrity_score']:.3f}")
    ...
    def _render_manual_relationships(self):
        ...
        with st.form("add_relationship"):
            ...
            with col1:
                ...
                source_table = st.selectbox("Source Table", table_names, key="source_table")
                
                if source_table:
                    source_columns = self._get_table_columns(source_table)
                    source_column = st.selectbox("Source Column", source_columns, key="source_column")
            
            with col2:
                st.write("**Target Table**")
                target_table = st.selectbox("Target Table", table_names, key="target_table")
                
                if target_table:
                    target_columns = self._get_table_columns(target_table)
                    target_column = st.selectbox("Target Column", target_columns, key="target_column")
            ...
            with col3:
                enforce_referential_integrity = st.checkbox("Enforce Referential Integrity")
            ...
            if st.form_submit_button("➕ Add Relationship", type="primary"):
                if all([source_table, source_column, target_table, target_column]):
                    self._add_manual_relationship(
                        ...
                        relationship_type, cardinality, enforce_referential_integrity, description
                    )
        ...
        if st.session_state.manual_relationships:
            ...
            for i, rel in enumerate(st.session_state.manual_relationships):
                with st.expander(
                    ...
                    with col2:
                        st.write(f"**Enforce Integrity:** {'Yes' if rel['enforce_referential_integrity'] else 'No'}")
                        st.write(f"**Added:** {rel['created_at']}")
                    
                    with col3:
                        if st.button(f"🗑️ Remove", key=f"remove_manual_{i}"):
    ...
    def _detect_relationships(self, method: str, confidence_threshold: float):
        ...
        for rel in detected_relationships:
            ...
            if rel_key not in seen_relationships and rel['confidence'] >= confidence_threshold:
                unique_relationships.append(rel)
                seen_relationships.add(rel_key)
        
        st.session_state.detected_relationships = unique_relationships
    
    def _detect_relationships_by_name_patterns(self, confidence_threshold: float) -> List[Dict]:
                ...
                for pattern in fk_patterns:
                    ...
                    if match:
                        ...
                        for target_table in self.registered_tables:
                            if (target_table['name'].lower() == referenced_table.lower() or
                                referenced_table.lower() in target_table['name'].lower()):
                                
                                # Look for primary key column in target table
                                target_columns = self._get_table_columns(target_table['name'])
                                for target_col in target_columns:
                                    if target_col.lower() in ['id', 'pk', f"{target_table['name']}_id".lower()]:
                                        confidence = self._calculate_name_pattern_confidence(
    ...
    def _detect_relationships_by_data_analysis(self, confidence_threshold: float) -> List[Dict]:
            ...
            for target_table in self.registered_tables:
                ...
                for source_col in source_columns:
                    for target_col in target_columns:
                        ...
                        if not self._columns_potentially_related(source_col, target_col):
                            continue
                        
                        try:
                            # Sample data from both columns
                            source_data = self._sample_column_data(source_table['name'], source_col)
                            target_data = self._sample_column_data(target_table['name'], target_col)
                            
                            if source_data is None or target_data is None:
                                continue
                            
                            # Calculate relationship metrics
                            match_rate = self._calculate_match_rate(source_data, target_data)
                            cardinality = self._analyze_cardinality(source_data, target_data)
                            ...
                            if match_rate >= 0.1:  # At least 10% match rate
                                ...
                                if confidence >= confidence_threshold:
                                    relationships.append({
                                        ...
                                        'target_table': target_table['name'],
                                        'target_column': target_col,
                                        'relationship_type': self._infer_relationship_type(cardinality),
                                        'confidence': confidence,
    ...
    def _generate_schema_diagram(self, layout_algorithm: str, show_columns: bool, highlight_relationships: bool):
        ...
        try:
            ...
            if highlight_relationships:
                for edge in G.edges():
                    ...
                    fig.add_trace(go.Scatter(
                        ...
                        y=[mid_y],
                        mode='text',
                        text=[f"{edge_data['source_column']}→{edge_data['target_column']}"],
            ...
            for node in G.nodes():
                ...
                # Node text
                if show_columns:
                    columns_text = "<br>".join([f"• {col}" for col in node_data['columns'][:10]])
                    if len(node_data['columns']) > 10:
                        columns_text += f"<br>... and {len(node_data['columns']) - 10} more"
                ...
                relationship_count = len([rel for rel in all_relationships 
    ...
    def _validate_data_integrity(self, relationship: Dict, sample_size: int) -> Dict:
        ...
        try:
            ...
            null_rate = (len(source_data) - len(source_values)) / len(source_data) if len(source_data) > 0 else 0
            
            # Calculate integrity score
            integrity_score = match_rate * (1 - null_rate * 0.5)  # Penalize high null rates
            ...
            if match_rate < 0.8:
                ...
                recommendations.append("Check for data inconsistencies or missing references")
            
            if null_rate > 0.2:
                issues.append(f"High null rate: {null_rate:.1%}")
                recommendations.append("Consider allowing NULL values or filling missing data")
            
            orphaned_values = source_values - target_values
            if orphaned_values:
                issues.append(f"{len(orphaned_values)} orphaned values found")
                recommendations.append("Review orphaned records and fix referential integrity")
    ...
    def _validate_referential_integrity(self, relationship: Dict, sample_size: int) -> Dict:
    ...
    def _validate_statistical_relationship(self, relationship: Dict, sample_size: int) -> Dict:
    ...
    def _render_table_statistics(self):
        ...
        for table in self.registered_tables:
            ...
            relationship_count = len([
                rel for rel in st.session_state.detected_relationships + st.session_state.manual_relationships
                if rel['source_table'] == table['name'] or rel['target_table'] == table['name']
            ])
    ...
    def _sample_column_data(self, table_name: str, column_name: str, sample_size: int = 1000) -> Optional[pd.Series]:
    ...
    def _calculate_name_pattern_confidence(self, source_col: str, target_col: str, pattern: str) -> float:
        ...
        if source_col.lower().replace('_id', '').replace('id', '') == target_col.lower():
            base_confidence += 0.15
        
        # Boost for common FK naming conventions
        if any(keyword in source_col.lower() for keyword in ['_id', 'id_', '_key', 'fk_']):
    ...
    def _columns_potentially_related(self, col1: str, col2: str) -> bool:
        ...
        patterns = [
            ...
            (any(keyword in col1_lower for keyword in ['id', 'key', 'ref']) and 
             any(keyword in col2_lower for keyword in ['id', 'key', 'ref']))
        ]
        
        return any(patterns)
    
    def _calculate_match_rate(self, source_data: pd.Series, target_data: pd.Series) -> float:
    ...
    def _analyze_cardinality(self, source_data: pd.Series, target_data: pd.Series) -> str:
    ...
    def _calculate_data_analysis_confidence(self, match_rate: float, cardinality: str, 
    ...
    def _accept_relationship(self, index: int):
        ...
        if 0 <= index < len(st.session_state.detected_relationships):
            ...
            relationship['created_at'] = pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    def _add_manual_relationship(self, source_table: str, source_column: str, target_table: str, 
                               target_column: str, relationship_type: str, cardinality: str,
                               enforce_integrity: bool, description: str):
