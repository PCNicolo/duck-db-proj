...
class StreamingQueryDisplay:
    ...
    def show_progress(self, current: int, total: Optional[int] = None, message: str = "Loading..."):
        ...
        if self.progress_placeholder:
            if total:
                ...
                self.progress_placeholder.progress(
                    progress,
                    text=f"{message} ({current:,}/{total:,} rows)"
                )
            else:
                # Indeterminate progress
                self.progress_placeholder.progress(
                    (current % 100) / 100,
                    text=f"{message} ({current:,} rows loaded)"
                )
    ...
    def show_cancel_button(self, cancel_callback: Callable):
        ...
        if self.cancel_placeholder and self.state.is_streaming:
            if self.cancel_placeholder.button("🛑 Cancel Query", type="secondary", use_container_width=True):
                        ...
                        if self.data_placeholder:
                            with self.data_placeholder.container():
                                if len(all_data[0]) > max_display_rows:
                                    st.warning(f"Showing first {max_display_rows:,} rows. Full results available after completion.")
                ...
                if self.data_placeholder:
                    with self.data_placeholder.container():
                        if len(final_data) > max_display_rows:
                            st.info(f"Dataset contains {len(final_data):,} rows. Showing first {max_display_rows:,} rows.")
                            st.dataframe(final_data.head(max_display_rows), use_container_width=True)
                        else:
...
class VirtualScrollDataFrame:
    ...
    def display(self):
        ...
        with col2:
            if st.button("◀️ Previous", disabled=st.session_state.virtual_scroll_page == 0):
        ...
        with col4:
            if st.button("Next ▶️", disabled=st.session_state.virtual_scroll_page >= self.total_pages - 1):
                st.session_state.virtual_scroll_page += 1
                st.rerun()
        
        with col5:
            if st.button("Last ⏭️", disabled=st.session_state.virtual_scroll_page >= self.total_pages - 1):
        ...
        # Display info
        st.caption(f"Showing rows {start_idx+1:,} to {end_idx:,} of {len(self.df):,} total rows")
    ...
    def display(self, df: pd.DataFrame):
        ...
        if rows_shown < total_rows:
            ...
            if remaining > self.increment:
                if st.button(f"Load all {remaining:,} remaining rows", type="secondary"):
