...
class EnhancedSQLEditor:
    ...
    def _render_schema_viewer(self):
        ...
        if st.button(
            "◀" if st.session_state.schema_expanded else "▶",
            key="toggle_schema",
        ...
        if st.session_state.schema_expanded:
            ...
            if self.db_connection and hasattr(st.session_state, "registered_tables"):
                for table in st.session_state.registered_tables:
                    ...
                    if (
                        schema_search
                        and schema_search.lower() not in table_name.lower()
                    ):
                        continue
                    ...
                    with st.expander(f"📋 {table_name}", expanded=False):
                        try:
                            ...
                            st.caption(
                                f"Rows: {table['rows']:,} | Cols: {table['columns']}"
                            )
            ...
            with col1:
                if st.button(
                    "➕ Expand All", key="expand_all", use_container_width=True
                ):
                    pass  # Streamlit doesn't support programmatic expander control
            with col2:
                if st.button(
                    "➖ Collapse All", key="collapse_all", use_container_width=True
                ):
                    pass
    ...
    def _render_toolbar(self):
        ...
        with col2:
            if st.button(
                "💾 Save", key="save_btn", help="Ctrl/Cmd + S", use_container_width=True
            ):
                self._save_query()
        ...
        with col6:
            if st.button(
                "⛶ Fullscreen", key="fullscreen_btn", use_container_width=True
            ):
    ...
    def _render_status_bar(self):
        ...
        with col3:
            ...
            if (
                hasattr(st.session_state, "query_result")
    ...
    def _render_query_history(self):
        ...
        history_search = st.text_input(
            "Search history", placeholder="Filter queries...", key="history_search"
        )
        ...
        if st.session_state.query_history:
            for i, item in enumerate(reversed(st.session_state.query_history[-20:])):
                ...
                if (
                    history_search
                    and history_search.lower() not in item["query"].lower()
                ):
                    continue
                ...
                with st.expander(f"🕐 {item['timestamp']}", expanded=False):
                    ...
                    with col1:
                        if st.button(
                            "📋", key=f"use_history_{i}", help="Use this query"
                        ):
                            st.session_state.editor_sql = item["query"]
                            st.rerun()
                    with col2:
                        if st.button(
                            "⭐", key=f"fav_history_{i}", help="Add to favorites"
                        ):
                            self._add_to_favorites(item["query"])
                    with col3:
                        if st.button(
                            "🗑️", key=f"del_history_{i}", help="Remove from history"
                        ):
    ...
    def _render_favorite_queries(self):
        ...
        if st.button(
            "⭐ Add Current Query", key="add_current_fav", use_container_width=True
        ):
        ...
        if st.session_state.favorite_queries:
            for i, fav in enumerate(st.session_state.favorite_queries):
                with st.expander(
                    f"⭐ {fav.get('name', f'Query {i+1}')}", expanded=False
                ):
                    st.code(fav["query"], language="sql")

                    # Name input for favorite
                    new_name = st.text_input(
                        "Name", value=fav.get("name", ""), key=f"fav_name_{i}"
                    )
                    ...
                    with col1:
                        if st.button(
                            "📋 Use", key=f"use_fav_{i}", use_container_width=True
                        ):
                            st.session_state.editor_sql = fav["query"]
                            st.rerun()
                    with col2:
                        if st.button(
                            "🗑️ Remove", key=f"del_fav_{i}", use_container_width=True
                        ):
            ...
            for line in lines:
                ...
                if line.upper().startswith(
                    ...
                    formatted_lines.append(
                        "    " * indent_level + line if indent_level > 0 else line
                    )
    ...
    def _show_history(self):
        ...
        if st.session_state.query_history:
            ...
            query = st.session_state.query_history[st.session_state.history_index][
                "query"
            ]
