...
class DataExporter:
    ...
    def __init__(self):
        ...
        if 'export_preferences' not in st.session_state:
            st.session_state.export_preferences = {
                ...
                'excel': {'sheet_name': 'Data Export', 'include_header': True, 'freeze_panes': True},
                'json': {'format': 'records', 'pretty_print': True, 'indent': 2}
            }
    
    def render_export_interface(self, df: pd.DataFrame, title: str = "Data Export") -> None:
        """
        ...
        with col1:
            if st.button("📥 Export Now", type="primary"):
                self._execute_export(df, export_format, export_options, export_data_config, title)
        
        with col2:
            if len(df) > 10000:
                if st.button("⏳ Background Export"):
                    self._start_background_export(df, export_format, export_options, export_data_config, title)
        
        with col3:
            if st.button("📋 Preview Export"):
                self._preview_export(df, export_format, export_options, export_data_config)
        
        # Export history and management
        self._render_export_history()
        self._render_background_exports_status()
    
    def _render_format_options(self, format_type: str, df: pd.DataFrame) -> Dict[str, Any]:
    ...
    def _render_csv_options(self) -> Dict[str, Any]:
        ...
        with col1:
            delimiter = st.selectbox(
                "Delimiter",
                [',', ';', '|', '\t'],
                index=[',', ';', '|', '\t'].index(st.session_state.export_preferences['csv']['delimiter']),
        ...
        with col2:
            quote_char = st.selectbox(
                "Quote Character",
                ['"', "'", 'none'],
                index=['"', "'", 'none'].index(st.session_state.export_preferences['csv']['quote_char'])
            )
            ...
            with col2:
                if export_scope == "Sample":
                    ...
                    sample_method = st.selectbox(
                        "Sample Method",
                        ["Random", "First N", "Last N", "Every Nth"]
                    )
            ...
            with col2:
                if column_mode in ["Selected Columns", "Exclude Columns"]:
                    selected_columns = st.multiselect(
                        "Choose Columns",
                        df.columns.tolist(),
                        default=df.columns.tolist() if column_mode == "Selected Columns" else []
                    )
                else:
    ...
    def _execute_export(self, df: pd.DataFrame, format_type: str, format_options: Dict, 
                       data_config: Dict, title: str):
        ...
        try:
            ...
            # Export data
            export_data, mime_type = self._export_to_format(filtered_df, format_type, format_options)
            ...
            if export_data:
                ...
                self._add_to_export_history(filename, format_type, len(filtered_df), format_options)
                
                st.success(f"✅ Export ready! {len(filtered_df):,} rows exported to {format_type.upper()}")
            
        except Exception as e:
            st.error(f"❌ Export failed: {str(e)}")
    
    def _preview_export(self, df: pd.DataFrame, format_type: str, format_options: Dict, data_config: Dict):
        ...
        try:
            ...
            st.write(f"Preview of first {preview_rows} rows ({len(filtered_df):,} total rows will be exported)")
            ...
            if format_type in ['csv', 'json', 'sql']:
                ...
                if format_type == 'csv':
                    csv_preview = sample_data.to_csv(index=format_options.get('index', False), **{
                        k: v for k, v in format_options.items() 
                    ...
                    sql_preview = self._generate_sql_preview(sample_data, format_options)
    ...
    def _apply_data_filters(self, df: pd.DataFrame, data_config: Dict) -> pd.DataFrame:
        ...
        if export_scope == "Sample":
            ...
            if all([date_column, start_date, end_date]) and date_column in filtered_df.columns:
        ...
        selected_columns = data_config.get('selected_columns', filtered_df.columns.tolist())
        ...
        if column_mode == "Selected Columns":
            ...
            columns_to_keep = [col for col in filtered_df.columns if col not in selected_columns]
            if columns_to_keep:
                filtered_df = filtered_df[columns_to_keep]
        
        return filtered_df
    
    def _export_to_format(self, df: pd.DataFrame, format_type: str, options: Dict) -> tuple:
        ...
        if format_type == 'csv':
            ...
            return self._export_to_excel(df, options), 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
        
        elif format_type == 'json':
            json_str = df.to_json(**{k: v for k, v in options.items() if k != 'include_dtypes'})
            ...
            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                ...
                for i in range(num_sheets):
                    ...
                    if options.get('apply_formatting'):
                        self._apply_excel_formatting(writer.sheets[sheet_name], sheet_df, options)
        else:
            # Single sheet export
            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                df.to_excel(
                ...
                # Apply formatting if requested
                if options.get('apply_formatting'):
                    self._apply_excel_formatting(writer.sheets[options['sheet_name']], df, options)
    ...
    def _generate_sql_statements(self, df: pd.DataFrame, options: Dict) -> str:
        ...
        if include_create_table:
            create_table = self._generate_create_table_statement(df, table_name, quote_identifiers)
        ...
        for i in range(0, len(df), batch_size):
            ...
            if values_list:
                insert_stmt = f"INSERT INTO {quote}{table_name}{quote} ({columns_str}) VALUES\n"
                insert_stmt += ',\n'.join(values_list) + ';'
                statements.append(insert_stmt)
        
        return '\n\n'.join(statements)
    
    def _generate_create_table_statement(self, df: pd.DataFrame, table_name: str, quote_identifiers: bool) -> str:
    ...
    def _start_background_export(self, df: pd.DataFrame, format_type: str, format_options: Dict, data_config: Dict, title: str):
    ...
    def _render_export_history(self):
        ...
        if st.session_state.export_history:
            with st.expander("📈 Export History"):
                for export_record in reversed(st.session_state.export_history[-10:]):  # Show last 10
            ...
            with st.expander("⏳ Background Exports Status"):
                for export_id, export_info in self.background_exports.items():
                    ...
                    with col2:
                        ...
                        st.write(f"{status_emoji.get(export_info['status'])} {export_info['status'].title()}")
                    with col3:
    ...
    def _add_to_export_history(self, filename: str, format_type: str, row_count: int, options: Dict):
