...
class AdvancedFilterBuilder:
    ...
    def _render_single_filter(self, df: pd.DataFrame, filter_config: Dict, index: int) -> Dict:
        ...
        with col1:
            ...
            filter_config['column'] = st.selectbox(
                "Column",
                df.columns.tolist(),
                index=list(df.columns).index(filter_config['column']) if filter_config['column'] in df.columns else 0,
                key=f"col_{index}"
            )
        ...
        with col2:
            ...
            current_op = filter_config['operator'] if filter_config['operator'] in operators else operators[0]
        ...
        with col3:
            ...
            filter_config['value'] = self._render_value_input(
                df, filter_config, index
            )
    ...
    def _render_value_input(self, df: pd.DataFrame, filter_config: Dict, index: int):
        ...
        if operator in ['is_null', 'is_not_null']:
            ...
            with col1:
                if 'int' in data_type or 'float' in data_type:
                    min_val = st.number_input(
                        "Min", 
                        value=float(df[column].min()) if pd.notna(df[column].min()) else 0.0,
                        key=f"min_{index}"
                    )
                else:
            ...
            with col2:
                if 'int' in data_type or 'float' in data_type:
                    max_val = st.number_input(
                        "Max", 
                        value=float(df[column].max()) if pd.notna(df[column].max()) else 0.0,
                        key=f"max_{index}"
                    )
                else:
            ...
            if 'int' in data_type:
                ...
                return st.number_input(
                    "Value",
                    value=float(current_value) if current_value and str(current_value).replace('.','').isdigit() else 0.0,
                    key=f"val_{index}"
                )
            elif 'datetime' in data_type:
                return st.date_input(
                    "Value",
                    value=pd.to_datetime(current_value).date() if current_value else date.today(),
                    key=f"date_{index}"
                )
    ...
    def _render_save_load_filters(self):
        ...
        with st.expander("💾 Save/Load Filters"):
            ...
            with col2:
                ...
                if saved_filters:
                    selected_filter = st.selectbox("Load Saved Filters", [""] + list(saved_filters.keys()))
                    if st.button("Load Filters") and selected_filter:
                        self._load_filter_set(selected_filter, saved_filters[selected_filter])
    ...
    def apply_filters(self, df: pd.DataFrame, filter_config: Dict[str, Any]) -> pd.DataFrame:
        """
    ...
    def _build_condition(self, df: pd.DataFrame, filter_def: Dict) -> Optional[pd.Series]:
        ...
        try:
            if operator == 'equals':
                ...
                return col_data.astype(str).str.contains(str(value), na=False, case=False)
                ...
                return col_data.isin(value) if value else pd.Series([False] * len(col_data))
            elif operator == 'not_in_list':
                return ~col_data.isin(value) if value else pd.Series([True] * len(col_data))
