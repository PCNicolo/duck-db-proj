...
class SmartPagination:
    ...
    def render_pagination_interface(self, df: pd.DataFrame, table_name: str = "data") -> Dict[str, Any]:
        """
        ...
        if pagination_config['mode'] == 'virtual_scroll':
            paginated_data = self._render_virtual_scroll(df, pagination_config, table_name)
        elif pagination_config['mode'] == 'infinite_scroll':
            paginated_data = self._render_infinite_scroll(df, pagination_config, table_name)
        else:  # standard pagination
            paginated_data = self._render_standard_pagination(df, pagination_config, table_name)
    ...
    def _render_pagination_controls(self, total_rows: int) -> Dict[str, Any]:
        ...
        with col2:
            ...
            if pagination_mode == 'virtual_scroll':
                ...
                page_size = st.selectbox(
                    ...
                    index=[25, 50, 100, 250, 500, 1000, 2500, 5000].index(
                        min(st.session_state.pagination_settings['page_size'], 5000)
                    ) if st.session_state.pagination_settings['page_size'] in [25, 50, 100, 250, 500, 1000, 2500, 5000] else 2,
        ...
        current_page = min(st.session_state.pagination_settings['current_page'], total_pages - 1)
        
        # Auto-select optimal pagination mode for large datasets
        if total_rows > 10000 and pagination_mode == 'standard':
            st.info(f"💡 Consider using Virtual Scroll for better performance with {total_rows:,} rows")
    ...
    def _render_standard_pagination(self, df: pd.DataFrame, config: Dict[str, Any], table_name: str) -> pd.DataFrame:
        """Render standard pagination interface."""
        # Navigation controls
        col1, col2, col3, col4, col5, col6 = st.columns([1, 1, 2, 1, 1, 1])
        
        with col1:
            if st.button("⏮️ First", disabled=config['current_page'] == 0, help="Go to first page"):
                st.session_state.pagination_settings['current_page'] = 0
                st.rerun()
        
        with col2:
            if st.button("◀️ Previous", disabled=config['current_page'] == 0, help="Go to previous page"):
                st.session_state.pagination_settings['current_page'] = max(0, config['current_page'] - 1)
                st.rerun()
        ...
        with col3:
            if config['jump_to_page_enabled']:
                ...
                display_options = [f"Page {i+1} of {config['total_pages']}" for i in page_options]
                ...
                st.write(f"Page {config['current_page'] + 1} of {config['total_pages']}")
        
        with col4:
            if st.button("Next ▶️", disabled=config['current_page'] >= config['total_pages'] - 1, help="Go to next page"):
                st.session_state.pagination_settings['current_page'] = min(config['total_pages'] - 1, config['current_page'] + 1)
                st.rerun()
        
        with col5:
            if st.button("Last ⏭️", disabled=config['current_page'] >= config['total_pages'] - 1, help="Go to last page"):
                st.session_state.pagination_settings['current_page'] = config['total_pages'] - 1
                st.rerun()
        
        with col6:
            # Page size info
            start_row = config['current_page'] * config['page_size'] + 1
            end_row = min((config['current_page'] + 1) * config['page_size'], config['total_rows'])
            st.caption(f"Rows {start_row:,}-{end_row:,} of {config['total_rows']:,}")
        
        # Get paginated data
        paginated_data = self._get_page_data(df, config['current_page'], config['page_size'], table_name)
        ...
        # Display data
        st.dataframe(paginated_data, use_container_width=True, height=min(600, len(paginated_data) * 35 + 100))
        ...
        if len(paginated_data) > 20:
            ...
            with col1:
                if st.button("◀️ Prev Page", disabled=config['current_page'] == 0, key="bottom_prev"):
                    st.session_state.pagination_settings['current_page'] = max(0, config['current_page'] - 1)
                    st.rerun()
            ...
            with col3:
                if st.button("Next Page ▶️", disabled=config['current_page'] >= config['total_pages'] - 1, key="bottom_next"):
                    st.session_state.pagination_settings['current_page'] = min(config['total_pages'] - 1, config['current_page'] + 1)
                    st.rerun()
        
        return paginated_data
    
    def _render_virtual_scroll(self, df: pd.DataFrame, config: Dict[str, Any], table_name: str) -> pd.DataFrame:
        ...
        virtual_data = self._get_virtual_scroll_data(df, scroll_position, config['page_size'], table_name)
    ...
    def _render_infinite_scroll(self, df: pd.DataFrame, config: Dict[str, Any], table_name: str) -> pd.DataFrame:
        ...
        if remaining_rows > 0:
            ...
            with col1:
                if st.button(f"📥 Load {min(config['page_size'], remaining_rows)} More Rows"):
                    st.session_state.infinite_scroll_loaded += config['page_size']
                    st.rerun()
        ...
        st.progress(progress, text=f"Loaded {loaded_rows:,} of {config['total_rows']:,} rows ({progress:.1%})")
        
        return current_data
    
    def _get_page_data(self, df: pd.DataFrame, page: int, page_size: int, table_name: str) -> pd.DataFrame:
    ...
    def _get_virtual_scroll_data(self, df: pd.DataFrame, start_pos: int, size: int, table_name: str) -> pd.DataFrame:
    ...
    def _prefetch_pages(self, df: pd.DataFrame, config: Dict[str, Any], table_name: str):
    ...
    def _update_performance_metrics(self, config: Dict[str, Any]):
        ...
        load_time = 0.1 if config['mode'] == 'virtual_scroll' else 0.3  # Simulated times
    ...
    def _render_performance_dashboard(self):
        ...
        with col3:
            avg_load_time = sum(perf['page_load_times']) / len(perf['page_load_times']) if perf['page_load_times'] else 0
        ...
        if perf['cache_hit_rate'] < 0.3:
            st.info("🔄 Low cache hit rate. Consider enabling prefetch or increasing page size for better performance.")
        
        if len(st.session_state.pagination_cache) > 30:
            st.info("💾 High cache usage. Cache will be automatically cleaned up to maintain performance.")
        
        if avg_load_time > 0.5:
            st.warning("⏱️ Slow page load times detected. Consider using virtual scrolling for large datasets.")
