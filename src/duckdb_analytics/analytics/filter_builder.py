...
class FilterBuilder:
    ...
    def __init__(self, db_connection, available_columns: List[Dict[str, Any]]):
        ...
        self.column_types = {
            col["name"]: col.get("type", "").lower() for col in available_columns
        }
    ...
    def render(self, table_name: str) -> Dict[str, Any]:
        ...
        filter_tab, agg_tab, query_tab = st.tabs(
            ["🔍 Filters", "📊 Aggregations", "📝 Generated Query"]
        )
    ...
    def _render_filter_section(self, table_name: str):
        ...
        with col1:
            if st.button("➕ Add Filter Condition", key=f"add_filter_{table_name}"):
                ...
                st.session_state[f"filter_conditions_{table_name}"].append(
                    new_condition
                )
                st.rerun()

        with col2:
            if conditions and st.button(
                "🗑️ Clear All Filters", key=f"clear_filters_{table_name}"
            ):
                st.session_state[f"filter_conditions_{table_name}"] = []
                st.rerun()

    def _render_filter_condition(
        self, table_name: str, index: int, condition: FilterCondition
    ):
        ...
        with st.container():
            ...
            with col1:
                # Column selector
                column_idx = (
                    self.column_names.index(condition.column)
                    if condition.column in self.column_names
                    else 0
                )
    ...
    def _render_aggregation_section(self, table_name: str):
        ...
        with col2:
            if aggregations and st.button(
                "🗑️ Clear All Aggregations", key=f"clear_aggs_{table_name}"
            ):
                st.session_state[f"aggregations_{table_name}"] = []
                st.rerun()

    def _render_aggregation_spec(
        self, table_name: str, index: int, agg: AggregationSpec
    ):
        ...
        with st.container():
            ...
            with col1:
                ...
                column_idx = (
                    available_cols.index(agg.column)
                    if agg.column in available_cols
                    else 0
                )
                selected_column = st.selectbox(
                    "Column",
    ...
    def _render_query_section(self, table_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            with col1:
                if st.button(
                    "▶️ Execute Query",
                    key=f"execute_builder_{table_name}",
                    type="primary",
                ):
    ...
    def _render_value_input(
        self, column: str, operator: OperatorType, current_value: Any, key: str
    ) -> Any:
        ...
        if operator in [OperatorType.IN, OperatorType.NOT_IN]:
            ...
            # Numeric input
            return st.number_input("Value", value=current_value or 0, key=key)
        elif any(t in col_type for t in ["date", "time", "timestamp"]):
            # Date input
            return st.date_input("Value", value=current_value, key=key)
        else:
            # Text input
            return st.text_input("Value", value=current_value or "", key=key)
    ...
    def _explain_query(self, table_name: str) -> str:
        ...
        if not aggregations and not group_by_cols:
            ...
            if group_by_cols:
                explanation.append(
                    f"📊 **Grouping data by**: {', '.join(group_by_cols)}"
                )
            if aggregations:
                agg_descriptions = []
                for agg in aggregations:
            ...
            for condition in conditions:
                if condition.operator == OperatorType.IS_NULL:
                    ...
                    filter_descriptions.append(
                        f"{condition.column} is between {condition.value} and {condition.value2}"
                    )
                else:
                    filter_descriptions.append(
                        f"{condition.column} {condition.operator.value} {condition.value}"
                    )
            explanation.append(
                f"🔍 **Filtering where**: {' and '.join(filter_descriptions)}"
            )
