...
def create_heatmap(df: pd.DataFrame, x: str, y: str, values: str, 
...
def create_treemap(df: pd.DataFrame, labels: str, values: str, 
                   parents: Optional[str] = None, config: Optional[Dict[str, Any]] = None) -> go.Figure:
...
def create_sankey(df: pd.DataFrame, source: str, target: str, value: str,
                  config: Optional[Dict[str, Any]] = None) -> go.Figure:
...
def create_box_plot(df: pd.DataFrame, x: Optional[str], y: str,
                   config: Optional[Dict[str, Any]] = None) -> go.Figure:
...
def create_gauge_chart(value: float, title: str, 
    ...
    threshold_colors = config.get('threshold_colors', {
        'low': 'red',
        'medium': 'yellow', 
        'high': 'green'
    })
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            'steps': [
                {'range': [min_val, max_val * 0.3], 'color': threshold_colors['low']},
                {'range': [max_val * 0.3, max_val * 0.7], 'color': threshold_colors['medium']},
                {'range': [max_val * 0.7, max_val], 'color': threshold_colors['high']}
            ],
            'threshold': {
...
def create_waterfall_chart(df: pd.DataFrame, x: str, y: str,
                          config: Optional[Dict[str, Any]] = None) -> go.Figure:
...
def create_radar_chart(df: pd.DataFrame, categories: List[str], values_col: str,
    ...
    if group_by:
        ...
        for group in groups:
            ...
            fig.add_trace(go.Scatterpolar(
                ...
                theta=group_data[categories[0]] if len(categories) == 1 else categories,
                fill='toself',
                name=str(group)
            ))
    else:
