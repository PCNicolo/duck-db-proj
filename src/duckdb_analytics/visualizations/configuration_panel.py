...
class ChartConfigurationPanel:
    ...
    def _render_basic_settings(self, current_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with st.expander("📊 Basic Settings", expanded=True):
            config["title"] = st.text_input(
                "Chart Title", value=current_config.get("title", ""), key="chart_title"
            )
    ...
    def _render_color_settings(self, current_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with st.expander("🎨 Colors & Theme"):
            config["color_scheme"] = st.selectbox(
                ...
                self.color_schemes,
                index=(
                    self.color_schemes.index(
                        current_config.get("color_scheme", "viridis")
                    )
                    if current_config.get("color_scheme") in self.color_schemes
                    else 0
                ),
                key="color_scheme",
            )
    ...
    def _render_axis_settings(self, current_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with st.expander("📏 Axes"):
            config["x_title"] = st.text_input(
                "X-Axis Title", value=current_config.get("x_title", ""), key="x_title"
            )

            config["y_title"] = st.text_input(
                "Y-Axis Title", value=current_config.get("y_title", ""), key="y_title"
            )
    ...
    def _render_gauge_settings(self, current_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with st.expander("🔢 Gauge Settings"):
            ...
            config["gauge_shape"] = st.selectbox(
                "Gauge Shape", ["angular", "bullet"], index=0, key="gauge_shape"
            )

        return config

    def _render_hierarchy_settings(
    ...
    def _render_data_settings(
        self, df: pd.DataFrame, current_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        with st.expander("📊 Data Settings"):
            ...
            if config["sort_data"]:
                ...
                config["sort_by"] = st.selectbox(
                    "Sort By Column", all_cols, index=0, key="sort_by"
                )
            ...
            if config["filter_data"]:
                config["filters"] = self._render_data_filters(
                    df, current_config.get("filters", {})
                )

        return config

    def _render_data_filters(
        self, df: pd.DataFrame, current_filters: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for col in df.columns:
            ...
            if col_type in ["int64", "float64"]:
                ...
                ):  # Only show for reasonable number of categories
    ...
    def apply_config(self, fig, config: Dict[str, Any]):
        ...
        if config.get("show_grid", True):
            layout_updates["xaxis"] = {
                "showgrid": True,
                "gridcolor": config.get("grid_color", "#e6e6e6"),
            }
        ...
        if "x_showticklabels" in config:
            layout_updates.setdefault("xaxis", {})["showticklabels"] = config[
                "x_showticklabels"
            ]
        if "y_showticklabels" in config:
            layout_updates.setdefault("yaxis", {})["showticklabels"] = config[
                "y_showticklabels"
            ]
