...
class SchemaExtractor:
    ...
    def _get_table_list(self) -> List[str]:
        ...
        if hasattr(self.conn, "execute"):
            ...
            df = (
                ...
                else pd.DataFrame(result.fetchall())
            )
        else:
    ...
    def _update_cache_key(self):
        ...
        cache_data = {
            table: len(schema.columns) for table, schema in self._schema_cache.items()
        }
    ...
    def _extract_schema(self) -> Dict[str, TableSchema]:
        ...
        try:
            ...
            for table_name in self._raw_schema_df["table_name"].unique():
                table_df = self._raw_schema_df[
                    self._raw_schema_df["table_name"] == table_name
                ]
                ...
                try:
                    ...
                    if hasattr(self.conn, "execute"):
                        ...
                        count_df = (
                            count_result.fetchdf()
                            if hasattr(count_result, "fetchdf")
                            else pd.DataFrame(count_result.fetchall())
                        )
                        table_schema.row_count = (
                            int(count_df.iloc[0]["cnt"]) if not count_df.empty else 0
                        )
                    else:
                    ...
                    if table_schema.row_count and table_schema.row_count > 0:
                        ...
                        if hasattr(self.conn, "execute"):
                            ...
                            sample_df = (
                                sample_result.fetchdf()
                                if hasattr(sample_result, "fetchdf")
                                else pd.DataFrame(sample_result.fetchall())
                            )
                        else:
                        ...
                        if (
                            ...
                            try:
                                ...
                                if hasattr(self.conn, "execute"):
                                    ...
                                    card_df = (
                                        card_result.fetchdf()
                                        if hasattr(card_result, "fetchdf")
                                        else pd.DataFrame(card_result.fetchall())
                                    )
                                    if not card_df.empty:
                                        table_schema.cardinality_stats[col.name] = int(
                                            card_df.iloc[0]["card"]
                                        )
                                else:
                                ...
                                logger.debug(
                                    f"Could not get cardinality for {table_name}.{col.name}: {e}"
                                )
    ...
    def format_for_llm(
        ...
        for table_name, table_schema in schema.items():
            ...
            for col in table_schema.columns:
                ...
                if (
                    col.name in table_schema.cardinality_stats
                    and context_level == "comprehensive"
                ):
            ...
            if table_schema.foreign_keys and context_level in [
                "standard",
                "comprehensive",
            ]:
                formatted_parts.append("\nRelationships:")
            ...
            if (
                include_samples
                and table_schema.sample_data
                and context_level != "minimal"
            ):
                formatted_parts.append("\nSample Data:")
                # Get column names
        ...
        if context_level == "comprehensive":
            ...
            formatted_parts.append(
                "- Use indexes on primary/unique columns for performance"
            )
    ...
    def validate_sql(self, sql_query: str) -> Tuple[bool, List[str]]:
        ...
        found_tables = re.findall(
            table_pattern, sql_query.upper().replace(sql_query, sql_query.upper())
        )
        ...
        for table in found_tables:
            if table.lower() not in available_tables:
                ...
                if similar:
                    errors.append(
                        f"Table '{table}' not found. Did you mean: {', '.join(similar[:3])}?"
                    )
                else:
                    errors.append(
                        f"Table '{table}' not found. Available tables: {', '.join(list(schema.keys())[:5])}"
                    )
        ...
        try:
            ...
            if "Catalog Error" in error_msg:
                ...
                if match:
                    ...
                    if similar:
                        errors.append(
                            f"Table '{table_name}' does not exist. Similar tables: {', '.join(similar[:3])}"
                        )
                    else:
                ...
                match = re.search(
                    r"Column ['\"]?(\w+)['\"]? (does not exist|not found)", error_msg
                )
                if match:
                    column_name = match.group(1)
                    errors.append(
                        f"Column '{column_name}' not found. Check column names in schema."
                    )
                else:
    ...
    def _find_similar_names(
        self, name: str, available_names: List[str], threshold: float = 0.6
    ) -> List[str]:
        """
