...
class ContextWindowManager:
    ...
    def prioritize_tables(self, query: str, schema: Dict, max_tables: int = 10) -> List[str]:
        """
    ...
    def truncate_context(self, context: str, target_tokens: Optional[int] = None) -> str:
        """
    ...
    def build_dynamic_prompt(self, base_prompt: str, query: str, schema_context: str) -> Tuple[str, Dict]:
        """
        ...
        if estimated_tokens > self.max_tokens:
            logger.warning(f"Prompt exceeds token limit ({estimated_tokens} > {self.max_tokens}), truncating...")
            # Truncate schema context
            reduced_schema = self.truncate_context(
                schema_context, 
                target_tokens=self.max_tokens - self.estimate_tokens('\n'.join(prompt_parts[:2] + prompt_parts[-2:]))
            )
    ...
    def compress_schema_context(self, schema_context: str, compression_level: str = 'standard') -> str:
        """
        ...
        if compression_level == 'minimal':
            ...
            for line in lines:
                if (line.startswith('#') or 
                    ...
                    (line.strip().startswith('-') and ':' in line and 'Sample' not in line)):
