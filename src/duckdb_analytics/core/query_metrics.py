...
class QueryMetricsCollector:
    ...
    def end_query(
        ...
        with self._lock:
            ...
            metric.execution_time = (metric.end_time - metric.start_time).total_seconds() * 1000
            ...
            # Log slow queries
            if self.log_slow_queries and metric.execution_time > self.slow_query_threshold_ms:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if recent_queries:
                recent_exec_times = [q.execution_time for q in recent_queries if not q.cache_hit]
...
class QueryProfiler:
    ...
    def get_optimization_suggestions(self, query_id: str) -> List[str]:
        ...
        if 'HASH JOIN' in plan.upper():
            suggestions.append("Large hash joins detected - consider optimizing join conditions")
...
class SlowQueryLogger:
    ...
    def _setup_file_logger(self):
        ...
        formatter = logging.Formatter(
            '%(asctime)s - SLOW_QUERY - %(message)s'
        )
    ...
    def analyze_patterns(self) -> Dict[str, Any]:
        ...
        patterns = {
            ...
            'avg_execution_time_ms': sum(q['execution_time_ms'] for q in self.slow_queries) / len(self.slow_queries),
            'max_execution_time_ms': max(q['execution_time_ms'] for q in self.slow_queries),
            'common_operations': {}
        }
        ...
        for query_info in self.slow_queries:
            ...
            if 'JOIN' in query:
                patterns['common_operations']['joins'] = patterns['common_operations'].get('joins', 0) + 1
            if 'GROUP BY' in query:
                patterns['common_operations']['group_by'] = patterns['common_operations'].get('group_by', 0) + 1
            if 'ORDER BY' in query:
                patterns['common_operations']['order_by'] = patterns['common_operations'].get('order_by', 0) + 1
