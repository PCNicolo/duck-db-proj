...
def sql_generator(mock_openai_client):
    ...
    with patch('src.duckdb_analytics.llm.sql_generator.OpenAI', return_value=mock_openai_client):
...
Table: customers (100 rows)
  ...
  - name (VARCHAR)
    ...
    def test_generate_sql_stream_with_schema(self, sql_generator, mock_openai_client):
        ...
        mock_openai_client.chat.completions.create.return_value = iter([mock_chunk1, mock_chunk2, mock_chunk3])
        
        schema_context = "Table: customers"
        
        # Generate SQL stream with schema
        result = list(sql_generator.generate_sql_stream("show customers", schema_context))
