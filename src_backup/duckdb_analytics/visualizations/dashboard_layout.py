...
class DashboardLayoutManager:
    ...
    def create_layout(
        self, name: str, layout_type: LayoutType, columns: int = 12, rows: int = 8
    ) -> DashboardLayout:
    ...
    def add_chart_to_layout(
        self, layout_id: str, chart_id: str, position: ChartPosition
    ) -> bool:
    ...
    def _render_grid_layout(
        self, layout: DashboardLayout, charts: Dict[str, go.Figure]
    ) -> None:
        ...
        for chart_pos in layout.charts:
            if chart_pos.chart_id in charts:
                ...
                with cols[col_index]:
                    ...
                    st.plotly_chart(
                        fig, use_container_width=True, key=f"chart_{chart_pos.chart_id}"
                    )
    ...
    def _render_tabs_layout(
        ...
        for i, (tab, chart_pos) in enumerate(zip(tabs, layout.charts, strict=False)):
            with tab:
                if chart_pos.chart_id in charts:
                    fig = charts[chart_pos.chart_id]
                    st.plotly_chart(
                        ...
                        key=f"tab_chart_{chart_pos.chart_id}",
                    )
    ...
    def _render_columns_layout(
        ...
        for col, chart_pos in zip(cols, layout.charts, strict=False):
            with col:
                if chart_pos.chart_id in charts:
                    fig = charts[chart_pos.chart_id]
                    ...
                    st.plotly_chart(
                        fig,
                        use_container_width=True,
                        key=f"col_chart_{chart_pos.chart_id}",
                    )
    ...
    def _render_rows_layout(
        ...
        for chart_pos in layout.charts:
            if chart_pos.chart_id in charts:
                ...
                fig.update_layout(title=chart_pos.title or fig.layout.title.text)
                st.plotly_chart(
                    fig, use_container_width=True, key=f"row_chart_{chart_pos.chart_id}"
                )
                st.divider()

    def _render_custom_layout(
        self, layout: DashboardLayout, charts: Dict[str, go.Figure]
    ) -> None:
            ...
            if len(row_charts) == 1:
                ...
                if chart_pos.chart_id in charts:
                    ...
                    st.plotly_chart(
                        fig,
                        use_container_width=True,
                        key=f"custom_chart_{chart_pos.chart_id}",
                    )
            else:
                # Multiple charts in row
                cols = st.columns([pos.width for pos in row_charts])

                for col, chart_pos in zip(cols, row_charts, strict=False):
                    with col:
                        if chart_pos.chart_id in charts:
                            fig = charts[chart_pos.chart_id]
                            fig.update_layout(
    ...
    def render_layout_builder(self) -> Optional[str]:
        ...
        with col1:
            ...
            if st.button("Create Layout", type="primary"):
                layout = self.create_layout(
                    layout_name, LayoutType(layout_type), columns, rows
                )
        ...
        with col2:
            ...
            if self.layouts:
                layout_options = {
                    f"{layout.name} ({layout.id})": layout.id
                    for layout in self.layouts.values()
                }

                selected = st.selectbox(
                    "Select Layout to Preview", list(layout_options.keys())
                )
    ...
    def create_subplot_dashboard(
        self, charts: Dict[str, go.Figure], layout_config: Dict[str, Any]
    ) -> go.Figure:
