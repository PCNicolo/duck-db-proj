...
class ChartRecommendationEngine:
    ...
    def recommend_charts(
        self, df: pd.DataFrame, max_recommendations: int = 5
        ...
        filtered_recs = [
            r for r in recommendations if r.score >= self.min_score_threshold
        ]
    ...
    def _analyze_columns(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for col in df.columns:
            col_info = {
                ...
                "is_categorical": df[col].nunique() < len(df) * 0.1
    ...
    def _detect_patterns(self, df: pd.DataFrame) -> Dict[str, bool]:
        ...
        patterns = {
            "has_time_series": any(
                col["is_datetime"] or col.get("potential_time", False)
                for col in columns.values()
            ),
            "has_categories": any(col["is_categorical"] for col in columns.values()),
            "has_correlations": len([c for c in columns.values() if c["is_numeric"]])
            >= 2,
            "has_distributions": any(
                col["is_numeric"] and col["unique_count"] > 10
                for col in columns.values()
            ),
    ...
    def _recommend_time_series(
        ...
        numeric_cols = [
            col for col, info in analysis["columns"].items() if info["is_numeric"]
        ]
    ...
    def _recommend_categorical(
        ...
        recommendations = []
        cat_cols = [
            col for col, info in analysis["columns"].items() if info["is_categorical"]
        ]
        numeric_cols = [
            col for col, info in analysis["columns"].items() if info["is_numeric"]
        ]
        ...
        if cat_cols and numeric_cols:
            ...
            recommendations.append(
                ChartRecommendation(
                    ...
                    config={"labels": cat_cols[0], "values": numeric_cols[0]},
        ...
        if len(numeric_cols) >= 2:
            ...
            recommendations.append(
                ChartRecommendation(
                    ...
                    config={"correlation_matrix": True, "columns": numeric_cols},
            ...
            if len(numeric_cols) >= 3:
                recommendations.append(
                    ChartRecommendation(
                        ...
                        score=0.75,
                        reason="Multiple numeric columns suitable for scatter matrix",
                        config={
    ...
    def _is_potential_time_column(self, series: pd.Series) -> bool:
        ...
        if series.dtype == "object":
            ...
            return any(
                indicator in series.name.lower() for indicator in time_indicators
            )
        return False

    def _detect_hierarchy_columns(
    ...
    def _detect_flow_data(self, df: pd.DataFrame, columns: Dict[str, Any]) -> bool:
    ...
    def _detect_kpi_data(self, df: pd.DataFrame, columns: Dict[str, Any]) -> bool:
