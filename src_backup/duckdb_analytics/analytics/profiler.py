...
class SummaryStatsGenerator:
    ...
    def generate_statistics(
        self, table_name: str, sample_size: int = 10000
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for column in columns:
                ...
                col_type = self._detect_column_type(
                    df[col_name], column.get("type", "")
                )

                if col_type == ColumnType.NUMERIC:
                    column_stats[col_name] = self._generate_numeric_stats(
                        df[col_name], col_name, table_name
                    )
                elif col_type == ColumnType.TEXT:
                    column_stats[col_name] = self._generate_text_stats(
                        df[col_name], col_name, table_name
                    )
                elif col_type == ColumnType.DATE:
                    column_stats[col_name] = self._generate_date_stats(
                        df[col_name], col_name, table_name
                    )
                elif col_type == ColumnType.BOOLEAN:
                    column_stats[col_name] = self._generate_boolean_stats(
                        df[col_name], col_name, table_name
                    )
                else:
                    column_stats[col_name] = self._generate_generic_stats(
                        df[col_name], col_name, table_name
                    )
    ...
    def _generate_numeric_stats(
        self, series: pd.Series, col_name: str, table_name: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            # Quality metrics
            completeness = (
                (stats["count"] - stats["missing"]) / stats["count"]
                if stats["count"] > 0
                else 0
            )
            ...
            if not numeric_series.dropna().empty:
                ...
                if abs(skewness) > 1:
                    distribution_type = (
                        "right-skewed" if skewness > 0 else "left-skewed"
                    )
            ...
            return {
                **stats,
                ...
                "skewness": (
                    numeric_series.skew() if not numeric_series.dropna().empty else 0
                ),
                "kurtosis": (
    ...
    def _generate_text_stats(
        ...
        try:
            ...
            stats = {
                ...
                "mode": (
                    text_series.mode().iloc[0] if not text_series.mode().empty else None
                ),
                "mode_frequency": (
                    (text_series == text_series.mode().iloc[0]).sum()
                    if not text_series.mode().empty
                    else 0
                ),
            }
            ...
            if not non_null_series.empty:
                ...
                patterns = {
                    ...
                    "contains_special_chars": non_null_series.str.contains(
                        r"[^a-zA-Z0-9\s]"
                    ).sum(),
    ...
    def _generate_date_stats(
        ...
        try:
            ...
            if (
                not isinstance(series.dtype, pd.DatetimeTZDtype)
                and series.dtype != "datetime64[ns]"
            ):
            ...
            return {**stats, "completeness": completeness, "uniqueness": uniqueness}

        except Exception as e:
            logger.error(f"Error generating date stats for {col_name}: {str(e)}")
            return self._generate_generic_stats(series, col_name, table_name)

    def _generate_boolean_stats(
        self, series: pd.Series, col_name: str, table_name: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                **stats,
                "completeness": completeness,
                "uniqueness": (
                    2 / stats["count"] if stats["count"] > 0 else 0
                ),  # Boolean has max 2 unique values
            }
...
class DataProfiler:
    ...
    def profile_table(self, table_name: str, sample_size: int = 10000) -> DataProfile:
        ...
        try:
            ...
            # Generate statistics
            stats_result = self.stats_generator.generate_statistics(
    ...
    def _extract_patterns(self, col_stats: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "outlier_count" in col_stats and col_stats.get("count", 0) > 0:
            ...
            if outlier_ratio > 0.1:
                patterns["outliers"] = (
                    f"High outlier count: {col_stats['outlier_count']}"
                )

        # Text patterns
        if "contains_digits" in col_stats:
            total_count = col_stats.get("count", 0) - col_stats.get(
                "missing", 0
            )  # Use non-null count
            if total_count > 0:
    ...
    def _calculate_overall_quality(
        ...
        completeness_scores = [
            cp.quality_metrics.get("completeness", 0) for cp in column_profiles
        ]
        uniqueness_scores = [
            cp.quality_metrics.get("uniqueness", 0) for cp in column_profiles
        ]
        validity_scores = [
            cp.quality_metrics.get("validity", 0) for cp in column_profiles
        ]
        ...
        return {
            ...
            "total_score": (
                sum(completeness_scores) + sum(uniqueness_scores) + sum(validity_scores)
            )
            / (3 * len(column_profiles)),
        }

    def _analyze_relationships(
        self, table_name: str, column_profiles: List[ColumnProfile]
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            numeric_columns = [
                cp.name for cp in column_profiles if cp.type == ColumnType.NUMERIC
            ]
                ...
                for i in range(len(numeric_columns)):
                    for j in range(i + 1, len(numeric_columns)):
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Error calculating correlation between {col1} and {col2}: {str(e)}"
                            )
