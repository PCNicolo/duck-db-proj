...
def create_progress_bar(
    ...
    if progress.is_complete:
        ...
        with col1:
            ...
            st.progress(progress.current_progress, text=progress_text)

        with col2:
            if show_cancel and cancel_callback:
                if st.button(
                    "Cancel", key=f"cancel_{progress.query_id}", type="secondary"
                ):
                    cancel_callback()
...
def create_spinner_with_elapsed_time(
    message: str = "Processing...", estimated_time: Optional[float] = None
) -> "ElapsedTimeSpinner":
    """
...
def show_row_count_indicator(current_rows: int, estimated_total: Optional[int] = None):
    ...
    if estimated_total:
        ...
        st.progress(
            progress, text=f"Loading rows: {current_rows:,} / {estimated_total:,}"
        )
    else:
...
def create_time_estimation_display(
    query: str, historical_times: Optional[list] = None
) -> float:
    """
...
def create_cancel_mechanism(cancel_callback: Callable, query_id: str) -> bool:
    """
